...
class Model(nn.Module):
    ...
    def build(self):
        ...
        self.rnn_cell1=nn.LSTMCell(self.rnn_hid_size,self.rnn_hid_size)
        ...
        self.htoh0=nn.Linear(self.rnn_hid_size  , self.rnn_hid_size*2)
    ...
    def get_hidden(self, data, direct):
        ...
        for t in range(SEQ_LEN):
            ...
            h1,c1=self.rnn_cell1(h,(h1,c1))
            h0,c0=self.rnn_cell1(h1,(h0,c0))
    ...
    def forward(self, data, direct):
        ...
        hidden_backward= self.get_hidden(data, 'backward')
        hidden_backward=hidden_backward[::-1]
        ...
        for t in range(SEQ_LEN):
            ...
            gama=self.temp_decay_x(d)
            x_h = self.hist_reg(h)
            z=m*x+(1-m)*x_h
            x_f = self.feat_reg(z)

            alpha = F.sigmoid(self.weight_combine(torch.cat([m, gama], dim = 1)))

            x_c = alpha * x_h + (1 - alpha)*x_f
